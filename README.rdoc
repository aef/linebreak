...
The default output encoding is unix. You can also choose mac and windows.
  ...
  linebreak -o windows unix.txt windows.txt

If no target file is specified the output will be sent to STDOUT.

  linebreak -o windows mac.txt > windows.txt

You can set the default with the environment variable LINEBREAK_OUTPUT.

  export LINEBREAK_OUTPUT=mac

  linebreak windows.txt mac.txt
...
deprecated. Modern Apple machines also use :unix linebreak encoding.
  ...
  Aef::Linebreak.convert(windows_string, :unix) #=> "Abcdef\nAbcdef\nAbcdef"
